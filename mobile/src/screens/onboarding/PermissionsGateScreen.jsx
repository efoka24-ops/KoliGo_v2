...
export default function PermissionsGateScreen({ lang = 'fr', onGranted }) {
  ...
  const requestPermission = async () => {
    ...
    try {
      ...
      // Background permissions not available (web/simulator) — proceed anyway
      if (step === 1) onGranted();
      else setStep(1);
    } finally {
      setLoading(false);
      ...
      {denied && (
        <View style={{ backgroundColor: '#FEF2F2', borderRadius: 12, padding: 14, marginBottom: 20, width: '100%' }}>
          <Text style={{ fontFamily: `${fonts.ui}-SemiBold`, fontSize: 13, color: '#D8472A', textAlign: 'center' }}>
            {lang === 'en'
              ? 'Permission denied. Enable GPS in Settings → Apps → KoliGo → Location.'
              : 'Permission refusée. Activez le GPS dans Paramètres → Applications → KoliGo → Localisation.'}
          </Text>
        </View>
      )}
