...
export default function SignupScreen({ navigation, route }) {
  ...
  const canProceed = name.trim().length >= 2 && phone.trim().length >= 8 && email.includes('@') && gender !== '' && accepted;
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      setPendingUser({ name: name.trim(), phone: fullPhone, email: email.trim(), gender });
